file_name='different_filters/loudness_21.wav'; % output signal of phase 1
final_filename='different_filters/loudness_21_leastSquares_Order10_lpf_kaiser.wav';
disp('ORDER 100')
...
for n=1:8
    N=10;
    Fs=16000;
    least_squares_2=least_squares_order2(N,Fs,cutoff_1,cutoff_1,cutoff_2,cutoff_2);
   
    tic
    filtered_signal=filter(least_squares_2,y_resampled);
    toc
...
for i=1:rows
    ...
    sampling_freq = 16000;
    cutoff_freq=400;
    %kaiser_lpf(N,Fs,Fc,flag,Beta)
    flag='scale';
    beta=0.5;
    Hd = kaiser_lpf(N,sampling_freq,cutoff_freq,flag,beta);
    
    %add filtered envelope à
    %tic
